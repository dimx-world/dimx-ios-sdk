...
there (`Libs/NativeCore/*.xcframework`, regenerated by
`ios-app/NativeCore/build_nativecore.sh` on every app build) instead of the
published archives. `Package.swift` carries both sets of `dxcore` targets, one of
them commented out - move the `/*` and `//*` to switch:
