...
class Material
{
    ...
    var baseColorWeight = Float(0.0)
    var baseColorMap: Texture?
    var normalMap: Texture?
    
    var receiveLighting = true
    var sdfText = false
    ...
    static func setBoolParam(_ id: Int, _ paramName: String, _ value: Bool) {
        let material = Renderer.instance.materials[id]!
        if paramName == "receiveLighting" {
            material.receiveLighting = value
    ...
    static func setFloatParam(_ id: Int, _ paramName: String, _ value: Float) {
        let material = Renderer.instance.materials[id]!
        if paramName == "baseColorWeight" {
            material.baseColorWeight = value
    ...
    func initData(_ coreMat: UnsafeRawPointer, _ mesh: Mesh, _ numSkelJoints: Int) {
        ...
        let baseColTexPtr = Material_getTexture(coreMat, "baseColorMap")
        if baseColTexPtr != nil {
            baseColorMap = Renderer.instance.textures[Texture_nativeId(baseColTexPtr!)]
        }
        
        Material_getParamVec4(coreMat, "addColor", &addColor)
        Material_getParamVec4(coreMat, "multColor", &multColor)

        let normalTexPtr = Material_getTexture(coreMat, "normalMap")
        if normalTexPtr != nil {
            normalMap = Renderer.instance.textures[Texture_nativeId(normalTexPtr!)]
        }

        metalness = Material_getParamFloat(coreMat, "metalness")
        let metalnessTexPtr = Material_getTexture(coreMat, "metalnessMap")
        ...
        if Material_hasParam(coreMat, "sdfText") {
            sdfText = Material_getParamBool(coreMat, "sdfText")
        }
                     ...
                     shadowsPass: Bool)
    {
        ...
        fragUniforms.fBaseColorWeight = baseColorWeight
    ...
    func populateVertAttribsConstants(_ values: MTLFunctionConstantValues, _ mesh: Mesh) {
        ...
        tmp = sdfText
        values.setConstantValue(&tmp, type: MTLDataType.bool, index: FunctionConstant.FCSdfText.rawValue)
    }
