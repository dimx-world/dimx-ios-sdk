...
constant bool SdfText          [[function_constant(FCSdfText)]];
                                 ...
                                 texture2d<half, access::sample> roughnessMap [[texture(FTIRoughnessMap), function_constant(MPRoughnessMap)]])
{
    ...
    // (baseColorMap) stores a distance value per texel in the R channel; the glyph edge sits at
    // 0.5. Using the screen-space derivative (fwidth) for the smoothstep width keeps glyph edges
    // crisp at any scale / viewing distance, so a single baked size suffices. Mirrors sdf_text.fs.
    if (SdfText && VATexCoord && MPBaseColorMap) {
        float dist = float(baseColorMap.sample(linearSampler, in.texCoords).r);
        float w = fwidth(dist);
        float alpha = smoothstep(0.5 - w, 0.5 + w, dist);
        if (alpha <= 0.0) {
            discard_fragment();
        }
        float4 sdfColor = uniforms.fBaseColor;
        if (VAColor) {
            sdfColor *= in.color;
        }
        sdfColor.a *= alpha;
