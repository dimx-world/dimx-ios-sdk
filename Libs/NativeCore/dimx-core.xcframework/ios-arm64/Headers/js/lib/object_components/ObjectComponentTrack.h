...
template <typename T>
class ObjectComponentTrack
{
public:
    ObjectComponentTrack(const Config& config) {
        if (config.isArray()) {
        ...
        switch (mType) {
            ...
            case AgentTrackType::CatmullRom: {
                int pointsPerSegment = config.get("pointsPerSegment", 4);
                bool loop = config.get("loop", false);
    ...
    T evaluate(float position) {
        ...
        float maxDist = mPositions[mLastIdx+1] - mPositions[mLastIdx];
        float frac = std::abs(maxDist) >= 0.000001f ? (position - mPositions[mLastIdx]) / maxDist : 1.f;
